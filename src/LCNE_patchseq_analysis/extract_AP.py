...
# Function to detect action potentials (spikes) in voltage traces
# def find_spikes(voltage_trace, threshold=10, half_window_size=15):
#     window_size = 2 * half_window_size + 1
#     local_maxima = (np.convolve(voltage_trace, np.ones(window_size), mode='valid') == voltage_trace[half_window_size:-half_window_size])
#     spike_indices = np.where(local_maxima & (voltage_trace > threshold))[0] + half_window_size
...
def get_data_files():
    ...
    for folder in files:
        ...
        nwb_files.extend([os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.nwb') and 'spikes' not in f])
        json_files.extend([os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.json') and 'EPHYS_QC_V3' in f and 'input' in f])
    ...
    for i, (nwb_file, json_file) in enumerate(zip(nwb_files, json_files)):
        ...
        with h5py.File(nwb_file, 'r') as hdf:
            ...
            for trial in trial_numbers:
                ...
                spike_waveforms = [raw_voltage[t-500:t+1500] for t in spike_times if t > 500 and t+1500 < len(raw_voltage)]
                if spike_waveforms:
