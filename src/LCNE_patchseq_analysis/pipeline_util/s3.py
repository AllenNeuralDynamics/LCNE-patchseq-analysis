...
from LCNE_patchseq_analysis import IN_CODEOCEAN
...
# Local dataset root in CodeOcean (mirrors S3 bucket layout)
CO_DATA_DIRECTORY = "/data/LCNE-patchseq-ephys"
...
def get_public_efel_cell_level_stats():
    ...
    if IN_CODEOCEAN:
        local_path = os.path.join(CO_DATA_DIRECTORY, "efel", "cell_stats", "cell_level_stats.csv")
        if os.path.exists(local_path):
            logger.info(f"[CodeOcean] Loading cell level stats from {local_path}")
...
def get_public_representative_spikes(
    ...
    if IN_CODEOCEAN:
        local_path = os.path.join(CO_DATA_DIRECTORY, "efel", "cell_stats", filename)
        if os.path.exists(local_path):
            logger.info(f"[CodeOcean] Loading spike waveforms from {local_path}")
...
def get_public_seq_preselected() -> pd.DataFrame:
    ...
    """
    if IN_CODEOCEAN:
        local_path = os.path.join(CO_DATA_DIRECTORY, "seq", "seq_preselected.csv")
        if os.path.exists(local_path):
            logger.info(f"[CodeOcean] Loading sequencing data from {local_path}")
            return pd.read_csv(local_path)
...
def get_public_mapmycells(filename="mapmycells_20250618.csv"):
    ...
    if IN_CODEOCEAN:
        local_path = os.path.join(CO_DATA_DIRECTORY, "seq", filename)
        if os.path.exists(local_path):
            logger.info(f"[CodeOcean] Loading MapMyCells data from {local_path}")
            with open(local_path, "rb") as f:
                return _parse_mapmycells(f)
...
def get_public_morphology(filename="LC_patchseq_RawFeatureWide.csv"):
    ...
    """
    if IN_CODEOCEAN:
        local_path = os.path.join(CO_DATA_DIRECTORY, "morphology", filename)
        if os.path.exists(local_path):
            logger.info(f"[CodeOcean] Loading morphology data from {local_path}")
            return pd.read_csv(local_path)
