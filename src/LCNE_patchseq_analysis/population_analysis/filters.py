...
q_fluorescence_has_data = '`jem-status_reporter` == `jem-status_reporter`'  # True for non-NaN values
q_marker_gene_any_positive = '`gene_Dbh (log_normed)` > 0 or `gene_Th (log_normed)` > 0 or `gene_Slc18a2 (log_normed)` > 0 or `gene_Slc6a2 (log_normed)` > 0'
q_marker_gene_all_positive = '`gene_Dbh (log_normed)` > 0 and `gene_Th (log_normed)` > 0 and `gene_Slc18a2 (log_normed)` > 0 and `gene_Slc6a2 (log_normed)` > 0'
q_marker_gene_dbh_positive = '`gene_Dbh (log_normed)` > 0'
q_marker_gene_has_data = '`gene_Dbh (log_normed)` == `gene_Dbh (log_normed)`'  # True for non-NaN values
q_mapmycells_dbh = 'mapmycells_subclass_name.str.contains("DBH", case=False, na=False)'
q_mapmycells_has_data = 'mapmycells_subclass_name == mapmycells_subclass_name'  # True for non-NaN values
