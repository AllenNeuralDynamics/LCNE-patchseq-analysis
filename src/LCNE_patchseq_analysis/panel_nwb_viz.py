...
class PatchSeqNWBApp(param.Parameterized):
    ...
    def create_sweep_panel(self, ephys_roi_id=""):
        ...
        plot_panel = pn.bind(PatchSeqNWBApp.update_plot, raw=raw_this_cell, sweep=slider.param.value)
        ...
        tab_sweeps.style.apply(
            ...
            PatchSeqNWBApp.highlight_selected_rows,
            highlight_subset=raw_this_cell.df_sweeps.query("passed != passed")["sweep_number"].tolist(),
            color="salmon",
            fields=["passed"],
            axis=1,
        ).apply(
        ...
        sweep_msg = pn.bind(PatchSeqNWBApp.get_qc_message, sweep=slider.param.value, df_sweeps=raw_this_cell.df_sweeps)
    ...
    def create_cell_selector_panel(self):
        ...
        def update_sweep_view_from_table(event):
            if event.new:
                selected_index = event.new[0]
                self.data_holder.ephys_roi_id = str(int(self.df_meta.iloc[selected_index]["ephys_roi_id"]))
    ...
    def main_layout(self):
        ...
        pane_one_cell = pn.bind(self.create_sweep_panel, ephys_roi_id=self.data_holder.param.ephys_roi_id)
